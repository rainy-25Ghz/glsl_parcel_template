...
void main()
{
    ...
    // Basic normalization.
    vec2 uvCoord=gl_FragCoord.xy/u_resolution;//左下角为（0，0），右上角为（1，1）

    vec2 pixelSize = vec2(1.0) / u_resolution.xy;
    ...
    vec2  mousePos = vec2(0.63, 0.27); // Default position.
    ...
    {        
        ...
        direction += horizontal * (pixelSize.x * random() + uvCoord.x);
        direction += vertical * (pixelSize.y * random() + uvCoord.y);
